...
namespace Accounts.UI
{
    public partial class frmCurrentStock : MetroForm
    {
        ...
        private void GetItemsOpeningStock(Int64? Id)
        {
            ...
            if (Id == 0)
                Id = null;
            List<VoucherDetailEL> list = manager.GetItemsCurrentStockByCategory(Id, Operations.IdProject);
            if (list.Count > 0)
            {
        ...
        private void FillGrid(List<VoucherDetailEL> List)
        {
            ...
            if (List != null && List.Count > 0)
            {
                ...
                for (int i = 0; i < List.Count; i++)
                {
                    ...
                    grdCurrentStock.Rows[i].Cells[5].Value = List[i].Diemension;
                    grdCurrentStock.Rows[i].Cells[6].Value = List[i].TotalCartons;
                    grdCurrentStock.Rows[i].Cells[7].Value = List[i].BatchNo;
                    grdCurrentStock.Rows[i].Cells[8].Value = List[i].Expiry;
                    grdCurrentStock.Rows[i].Cells[9].Value = List[i].EngineNo;
                    grdCurrentStock.Rows[i].Cells[10].Value = List[i].ChasisNo;
                    grdCurrentStock.Rows[i].Cells[11].Value = List[i].VehicleModel;
                    if (List[i].ColorCode == 1)
                    {
                        grdCurrentStock.Rows[i].Cells[12].Value = "Red";
                    }
                    else if (List[i].ColorCode == 1)
                    {
                        grdCurrentStock.Rows[i].Cells[12].Value = "Black";
                    }
                    else if (List[i].ColorCode == 1)
                    {
                        grdCurrentStock.Rows[i].Cells[12].Value = "Blue";
                    }
                    else if (List[i].ColorCode == 1)
                    {
                        grdCurrentStock.Rows[i].Cells[12].Value = "Silver";
                    }
                    grdCurrentStock.Rows[i].Cells[13].Value = List[i].VehicleNo;
                    grdCurrentStock.Rows[i].Cells[14].Value = List[i].FirstIMEI;
                    grdCurrentStock.Rows[i].Cells[15].Value = List[i].SecondIMEI;


                    grdCurrentStock.Rows[i].Cells[16].Value = CommonFunctions.RemoveTrailingZeros(List[i].Units);
                    grdCurrentStock.Rows[i].Cells[17].Value = CommonFunctions.RemoveTrailingZeros(List[i].Boxes);
                    grdCurrentStock.Rows[i].Cells[18].Value = CommonFunctions.RemoveTrailingZeros(List[i].Pieces);
                    grdCurrentStock.Rows[i].Cells[19].Value = CommonFunctions.RemoveTrailingZeros(List[i].Bonus);
                    grdCurrentStock.Rows[i].Cells[20].Value = CommonFunctions.RemoveTrailingZeros(List[i].Qty);
                    grdCurrentStock.Rows[i].Cells[21].Value = CommonFunctions.RemoveTrailingZeros(List[i].UnitPrice);
                    grdCurrentStock.Rows[i].Cells[22].Value = CommonFunctions.RemoveTrailingZeros(List[i].TotalAmount);
                    grdCurrentStock.Rows[i].Cells[23].Value = List[i].SaleTaxPercentage + "%";
                    grdCurrentStock.Rows[i].Cells[24].Value = List[i].SaleTaxFlatAmount;
                    grdCurrentStock.Rows[i].Cells[25].Value = List[i].OtherTaxPercentage + "%";
                    grdCurrentStock.Rows[i].Cells[26].Value = List[i].OtherTaxFlatAmount;
                    grdCurrentStock.Rows[i].Cells[27].Value = List[i].NetAmount;
            ...
            if (list.Count > 0)
            {
                ...
                if (objActiveType != null)
                {
                    ...
                    else if (objActiveType.SoftwareType == "Distribution Trading")
                    {
                        ...
                        grdCurrentStock.Columns["colItemName"].Width = 250;
                        grdCurrentStock.Columns["colpacking"].Width = 70;
                        grdCurrentStock.Columns["colQty"].Width = 70;
                        grdCurrentStock.Columns["colUnitPrice"].Width = 80;
                    }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < grdCurrentStock.Rows.Count - 1; i++)
            {
                ...
                oelCurrentStock.Units = Validation.GetSafeDecimal(grdCurrentStock.Rows[i].Cells["colQty"].Value);
                oelCurrentStock.Boxes = Validation.GetSafeDecimal(grdCurrentStock.Rows[i].Cells["colBoxes"].Value);
                oelCurrentStock.Pieces = Validation.GetSafeDecimal(grdCurrentStock.Rows[i].Cells["colPieces"].Value);
                oelCurrentStock.Qty = Validation.GetSafeDecimal(grdCurrentStock.Rows[i].Cells["colFinalQuantity"].Value);
                ...
                oelCurrentStock.SaleTaxPercentage = Validation.GetSafeDecimal(grdCurrentStock.Rows[i].Cells["colSaleTaxPercentage"].Value.ToString().Replace('%', ' '));
                oelCurrentStock.SaleTaxFlatAmount = Validation.GetSafeDecimal(grdCurrentStock.Rows[i].Cells["colSaleTaxFlat"].Value);
                oelCurrentStock.OtherTaxPercentage = Validation.GetSafeDecimal(grdCurrentStock.Rows[i].Cells["colExtraTaxPercentage"].Value.ToString().Replace('%', ' '));
                oelCurrentStock.OtherTaxFlatAmount = Validation.GetSafeDecimal(grdCurrentStock.Rows[i].Cells["colExtraTaxFlat"].Value);

                oelCurrentStock.NetAmount = Validation.GetSafeDecimal(grdCurrentStock.Rows[i].Cells["colTaxableAmount"].Value);
        ...
        {
            var manager = new ItemsBLL();
            //decimal CalculateQuantity = 0;
            string[] QuantityArray = Validation.GetSafeString(grdCurrentStock.Rows[e.RowIndex].Cells["colDimension"].Value).Split('*');
            if (e.ColumnIndex == 16)
            {
                if (QuantityArray[1] == "0")
                {
                    if (grdCurrentStock.Rows[e.RowIndex].Cells["colPieces"].Value == null || Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colPieces"].Value) == 0)
                    {
                        grdCurrentStock.Rows[e.RowIndex].Cells["colFinalQuantity"].Value = (Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(QuantityArray[2]));
                    }
                    else if (grdCurrentStock.Rows[e.RowIndex].Cells["colPieces"].Value != null && Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colPieces"].Value) > 0)
                    {
                        grdCurrentStock.Rows[e.RowIndex].Cells["colFinalQuantity"].Value = (Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(QuantityArray[2])) + Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colPieces"].Value);
                    }
                }
                else if (Validation.GetSafeDecimal(QuantityArray[1]) > 0)
                {
                    grdCurrentStock.Rows[e.RowIndex].Cells["colFinalQuantity"].Value = (Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(QuantityArray[1]) * Validation.GetSafeDecimal(QuantityArray[2])) + (Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colBoxes"].Value) * Validation.GetSafeDecimal(QuantityArray[2]) + Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colPieces"].Value));
                }
                ...
                grdCurrentStock.Rows[e.RowIndex].Cells["colTotalAmount"].Value = Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colFinalQuantity"].Value) *
            ...
            else if (e.ColumnIndex == 17)
            {
                if (QuantityArray[1] == "0")
                {
                    if (Validation.GetSafeString(grdCurrentStock.Rows[e.RowIndex].Cells["colBoxes"].Value) != "" && Validation.GetSafeString(grdCurrentStock.Rows[e.RowIndex].Cells["colBoxes"].Value) != "0")
                    {
                        MessageBox.Show("This Product Is Not Box Containing Item...");
                        grdCurrentStock.Rows[e.RowIndex].Cells["colBoxes"].Value = "";
                        return;
                    }
                }
                else
            ...
            else if (grdCurrentStock.Columns[e.ColumnIndex].Name == "colSaleTaxPercentage")
            {
                if (grdCurrentStock.Rows[e.RowIndex].Cells["colSaleTaxPercentage"].Value != null)
                {
                    string CellValue = Validation.GetSafeString(grdCurrentStock.Rows[e.RowIndex].Cells["colSaleTaxPercentage"].Value);
                    ...
                    {
                        grdCurrentStock.Rows[e.RowIndex].Cells["colSaleTaxPercentage"].Value = CellValue + "%";
                        grdCurrentStock.Rows[e.RowIndex].Cells["colSaleTaxFlat"].Value = ((Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colTotalAmount"].Value) / 100) * Validation.GetSafeDecimal(CellValue));
                        grdCurrentStock.Rows[e.RowIndex].Cells["colTaxableAmount"].Value = Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colTotalAmount"].Value) + (Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colSaleTaxFlat"].Value) + Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colExtraTaxFlat"].Value));

                    }
                    else
                    {
                        grdCurrentStock.Rows[e.RowIndex].Cells["colTaxableAmount"].Value = Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colTotalAmount"].Value);
                        grdCurrentStock.Rows[e.RowIndex].Cells["colSaleTaxFlat"].Value = 0;
                    }
                }
                else
                {
            ...
            else if (grdCurrentStock.Columns[e.ColumnIndex].Name == "colExtraTaxPercentage")
            {

                if (grdCurrentStock.Rows[e.RowIndex].Cells["colExtraTaxPercentage"].Value != null)
                {
                    string CellValue = Validation.GetSafeString(grdCurrentStock.Rows[e.RowIndex].Cells["colExtraTaxPercentage"].Value);
                    if (CellValue.Contains('%'))
                    {
                        CellValue = CellValue.Substring(0, CellValue.Length - 1);
                    }
                    if (Validation.GetSafeDecimal(CellValue) > 0)
                    {
                        grdCurrentStock.Rows[e.RowIndex].Cells["colExtraTaxPercentage"].Value = CellValue + "%";
                        grdCurrentStock.Rows[e.RowIndex].Cells["colExtraTaxFlat"].Value = ((Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colTotalAmount"].Value) / 100) * Validation.GetSafeDecimal(CellValue));
                        grdCurrentStock.Rows[e.RowIndex].Cells["colTaxableAmount"].Value = Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colTotalAmount"].Value) + (Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colSaleTaxFlat"].Value) + Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colExtraTaxFlat"].Value));
                    }
                    else
                    {
                        grdCurrentStock.Rows[e.RowIndex].Cells["colTaxableAmount"].Value = Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colTotalAmount"].Value) + Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colSaleTaxFlat"].Value) + Validation.GetSafeDecimal(grdCurrentStock.Rows[e.RowIndex].Cells["colExtraTaxFlat"].Value);
                        grdCurrentStock.Rows[e.RowIndex].Cells["colExtraTaxFlat"].Value = 0;                        
                    }
