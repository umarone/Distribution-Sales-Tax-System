...
namespace Accounts.UI
{
    public partial class frmDistributionSalesReturn : MetroForm
    {
        ...
        string CustomerAccountNo, SalesAccountNo = "", CashAccountNo = "", EmpAccountNo = "0", EmpDeliveryAccountNo = "", SalesTaxAccountNo = "";
            ...
            if (list.Count > 0)
            {
                ...
                if (objActiveType != null)
                {
                    ...
                    else if (objActiveType.SoftwareType == "Distribution Trading")
                    {
                        ...
                        DgvStockSales.Columns["colQty"].Width = 60;
                        DgvStockSales.Columns["colUnitPrice"].Width = 75;

                        DgvSales.Columns["colSalesUOM"].Visible = false;
                        ...
                        DgvSales.Columns["colSalesCurrentStock"].Visible = false;

                        DgvSales.Columns["colSalesItemName"].Width = 180;
                        DgvSales.Columns["colSalesQuantity"].Width = 70;
                        DgvSales.Columns["colSalesBonusUnit"].Width = 50;
                        DgvSales.Columns["colSalesUnitPrice"].Width = 65;
                        DgvSales.Columns["colSalesAmount"].Width = 80;
                        DgvSales.Columns["colSalesDiscount"].Width = 85;
                        DgvSales.Columns["colSalesDiscountAmount"].Width = 85;
                    }
                }
            }
        }
                ...
                if (ValidateControls())
                {
                    if (ValidateBookPeriod())
                    {
                        ...
                        for (int i = 0; i < DgvStockSales.Rows.Count - 1; i++)
                        {
                            ...
                            string[] Diemension = Validation.GetSafeString(DgvStockSales.Rows[i].Cells["colDimension"].Value).Split('*');
                           ...
                           //oelSaleDetial.Discount = Validation.GetSafeDecimal(DgvStockSales.Rows[i].Cells["colNormalDiscountPercentage"].Value.ToString().Replace('%', ' '));
                            ...
                            oelSaleDetial.SaleTaxPercentage = Validation.GetSafeDecimal(DgvStockSales.Rows[i].Cells["colSaleTaxPercentage"].Value.ToString().Replace('%', ' '));
                            oelSaleDetial.SaleTaxFlatAmount = Validation.GetSafeDecimal(DgvStockSales.Rows[i].Cells["colNormalSaleTaxFlat"].Value);
                        ...
                        oelSaleTransaction.Description = "Sale Returned";
                        ...
                        oelSaleTaxTransaction.Description = "Tax Account Debited In Sale Return";
                        oelSaleTaxTransaction.Credit = 0;
                        oelSaleTaxTransaction.Debit = Validation.GetSafeDecimal(txtTotalSaleTaxAmount.Text); ;
        ...
        private void DgvSales_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            Int64 IdItem = Validation.GetSafeLong(ORow.Cells["colSalesIdItem"].Value);
            if (DgvStockSales.Rows.Count > 0)
            {
                for (int i = 0; i < DgvStockSales.Rows.Count; i++)
                {
                    if(IdItem == Validation.GetSafeLong(DgvStockSales.Rows[i].Cells["colIdItem"].Value))
                    {
                        MessageBox.Show("Row Already Been Added");
                        return;
                    }
                }
            }
            ...
            DgvStockSales.Rows[DgvStockSales.Rows.Count - 2].Cells["colDimension"].Value = ORow.Cells["colSalesDimension"].Value;
            ...
            DgvStockSales.Rows[DgvStockSales.Rows.Count - 2].Cells["colBoxes"].Value = ORow.Cells["colSalesBoxes"].Value;
            DgvStockSales.Rows[DgvStockSales.Rows.Count - 2].Cells["colPieces"].Value = ORow.Cells["colSalesPieces"].Value;
            DgvStockSales.Rows[DgvStockSales.Rows.Count - 2].Cells["colUnitPrice"].Value = ORow.Cells["colSalesUnitPrice"].Value;
            DgvStockSales.Rows[DgvStockSales.Rows.Count - 2].Cells["colAmount"].Value = ORow.Cells["colSalesDiscountAmount"].Value;
            DgvStockSales.Rows[DgvStockSales.Rows.Count - 2].Cells["colSaleTaxPercentage"].Value = ORow.Cells["colSalesTaxPercentage"].Value;
            DgvStockSales.Rows[DgvStockSales.Rows.Count - 2].Cells["colNormalSaleTaxFlat"].Value = ORow.Cells["colSaleTaxFlat"].Value;
            DgvStockSales.Rows[DgvStockSales.Rows.Count - 2].Cells["colExtraTaxPercentage"].Value = ORow.Cells["colSaleExtraTaxPercentage"].Value;
            DgvStockSales.Rows[DgvStockSales.Rows.Count - 2].Cells["colExtraTaxFlat"].Value = ORow.Cells["colSaleExtraTaxFlat"].Value;
            DgvStockSales.Rows[DgvStockSales.Rows.Count - 2].Cells["colNetAmount"].Value = ORow.Cells["colSalesNetAmount"].Value;
            
            for (int i = 0; i < DgvStockSales.Rows.Count - 1; i++)
            {
                BillingAmount += Validation.GetSafeDecimal(DgvStockSales.Rows[i].Cells["colAmount"].Value);
                //NormalDiscount += Validation.GetSafeDecimal(DgvStockSales.Rows[i].Cells["colSaleNormalDiscountFlat"].Value);
                //BulkDiscount += Validation.GetSafeDecimal(DgvStockSales.Rows[i].Cells["colSaleBulkDiscountFlat"].Value);
                ValueAfterDiscount += Validation.GetSafeDecimal(DgvStockSales.Rows[i].Cells["colAmount"].Value);
                TaxPayableFlat += Validation.GetSafeDecimal(DgvStockSales.Rows[i].Cells["colNormalSaleTaxFlat"].Value);
        ...
        {
            var manager = new ItemsBLL();
            decimal DiscTotalAmount = 0, SameQuantity = 0, AssignQuantity = 0, ClosingStock = 0;
            ...
            string Diemension = Validation.GetSafeString(DgvStockSales.Rows[e.RowIndex].Cells["colDimension"].Value);
            ...
            if (DgvStockSales.Columns[e.ColumnIndex].Name == "colQty")
            {
                if (Validation.GetSafeDecimal(DimsArray[1]) == 0)
                {
                    ConvertedQty = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * AssignQuantity) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value));
                }
                else
                {
                    ConvertedQty = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(DimsArray[1]) * Validation.GetSafeDecimal(DimsArray[2])) + Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value) * Validation.GetSafeDecimal(DimsArray[2]) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value));
                }
                ...
                if (DgvStockSales.Rows[e.RowIndex].Cells["colSaleTaxPercentage"].Value != null)
                {
                    string CellValue = Validation.GetSafeString(DgvStockSales.Rows[e.RowIndex].Cells["colSaleTaxPercentage"].Value);
                    ...
                    if (Validation.GetSafeDecimal(CellValue) > 0)
                    {
                        if (arrItemTaxInfo[0].ItemSaleTaxType == 1)
                        {
                            ...
                            DgvStockSales.Rows[e.RowIndex].Cells["colSaleTaxPercentage"].Value = "0%";
                            return;
                        }
                        else if (arrItemTaxInfo[0].ItemSaleTaxType == 2)
                        {
                            //DgvStockSales.Rows[e.RowIndex].Cells["colSalesTaxPercentage"].Value = CellValue + "%";
                            DgvStockSales.Rows[e.RowIndex].Cells["colNormalSaleTaxFlat"].Value = ConvertedQty * arrItemTaxInfo[0].ThirdScheduleTaxCalculationRatio;                           
                            DgvStockSales.Rows[e.RowIndex].Cells["colNetAmount"].Value = Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colAmount"].Value) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colNormalSaleTaxFlat"].Value) + Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colExtraTaxFlat"].Value));
                        }
                        else
                        {
                            //DgvStockSales.Rows[e.RowIndex].Cells["colSalesTaxPercentage"].Value = CellValue + "%";
                            DgvStockSales.Rows[e.RowIndex].Cells["colNormalSaleTaxFlat"].Value = ((Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colAmount"].Value) / 100) * Validation.GetSafeDecimal(CellValue));
                            DgvStockSales.Rows[e.RowIndex].Cells["colNetAmount"].Value = Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colAmount"].Value) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colNormalSaleTaxFlat"].Value) + Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colExtraTaxFlat"].Value));
                        }
                    }
                    else
            ...
            else if (DgvStockSales.Columns[e.ColumnIndex].Name == "colBoxes")
            {
                ...
                else if (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colBoxes"].Value) == 0 && Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) > 0)
                {
                    ConvertedQty = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * AssignQuantity) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value));
                }
                DgvStockSales.Rows[e.RowIndex].Cells["colAmount"].Value = Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colUnitPrice"].Value) * ConvertedQty;
                ...
                if (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value) > 0)
                {
                    ...
                    else
                    {
                        if (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) > 0)
                        {
                            if (Validation.GetSafeDecimal(DimsArray[1]) > 0)
                            {
                                ConvertedQty = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(DimsArray[1]) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value));
                            }
                            else
                            {
                                ConvertedQty = (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colQty"].Value) * Validation.GetSafeDecimal(DimsArray[2])) + (Validation.GetSafeDecimal(DgvStockSales.Rows[e.RowIndex].Cells["colPieces"].Value));
                            }
                        }
                        else
                        {
            ...
            else if (DgvStockSales.Columns[e.ColumnIndex].Name == "colSaleTaxPercentage")
            {
                if (DgvStockSales.Rows[e.RowIndex].Cells["colSaleTaxPercentage"].Value != null)
                {
                    string CellValue = Validation.GetSafeString(DgvStockSales.Rows[e.RowIndex].Cells["colSaleTaxPercentage"].Value);
        ...
        private void FillInvoiceSales(List<VoucherDetailEL> List)
        {
            ...
            if (List != null && List.Count > 0)
            {
                ...
                for (int i = 0; i < List.Count; i++)
                {
                    ...
                    DgvSales.Rows[i].Cells[5].Value = List[i].Diemension;
                    DgvSales.Rows[i].Cells[6].Value = List[i].TotalCartons;
                    DgvSales.Rows[i].Cells[7].Value = List[i].BatchNo;
                    DgvSales.Rows[i].Cells[8].Value = List[i].Expiry;
                    DgvSales.Rows[i].Cells[9].Value = List[i].EngineNo;
                    DgvSales.Rows[i].Cells[10].Value = List[i].ChasisNo;
                    DgvSales.Rows[i].Cells[11].Value = List[i].VehicleModel;
                    if (List[i].ColorCode == 1)
                    {
                        DgvSales.Rows[i].Cells[12].Value = "Red";
                    }
                    else if (List[i].ColorCode == 1)
                    {
                        DgvSales.Rows[i].Cells[12].Value = "Black";
                    }
                    else if (List[i].ColorCode == 1)
                    {
                        DgvSales.Rows[i].Cells[12].Value = "Blue";
                    }
                    else if (List[i].ColorCode == 1)
                    {
                        DgvSales.Rows[i].Cells[12].Value = "Silver";
                    }
                    DgvSales.Rows[i].Cells[13].Value = List[i].VehicleNo;
                    DgvSales.Rows[i].Cells[14].Value = List[i].FirstIMEI;
                    DgvSales.Rows[i].Cells[15].Value = List[i].SecondIMEI;

                    DgvSales.Rows[i].Cells[16].Value = CommonFunctions.RemoveTrailingZeros(List[i].CurrentStock);
                    DgvSales.Rows[i].Cells[17].Value = CommonFunctions.RemoveTrailingZeros(List[i].Units);
                    DgvSales.Rows[i].Cells[18].Value = CommonFunctions.RemoveTrailingZeros(List[i].Boxes);
                    DgvSales.Rows[i].Cells[19].Value = CommonFunctions.RemoveTrailingZeros(List[i].Pieces);
                    DgvSales.Rows[i].Cells[20].Value = CommonFunctions.RemoveTrailingZeros(List[i].Bonus);
                    DgvSales.Rows[i].Cells[21].Value = List[i].UnitPrice;
                    DgvSales.Rows[i].Cells[22].Value = List[i].Amount;
                    DgvSales.Rows[i].Cells[23].Value = List[i].Discount.ToString() + "%";
                    DgvSales.Rows[i].Cells[24].Value = List[i].DiscountAmount;
                    DgvSales.Rows[i].Cells[25].Value = List[i].BulkDiscountPercentage.ToString() + "%";
                    DgvSales.Rows[i].Cells[26].Value = List[i].BulkDiscountFlatAmount;
                    DgvSales.Rows[i].Cells[27].Value = List[i].FlatDiscount;
                    DgvSales.Rows[i].Cells[28].Value = List[i].BillAmountAfterDiscount;
                    DgvSales.Rows[i].Cells[29].Value = List[i].SaleTaxPercentage;
                    DgvSales.Rows[i].Cells[30].Value = List[i].SaleTaxFlatAmount;
                    DgvSales.Rows[i].Cells[31].Value = List[i].OtherTaxPercentage;
                    DgvSales.Rows[i].Cells[32].Value = List[i].OtherTaxFlatAmount;
                    DgvSales.Rows[i].Cells[33].Value = List[i].NetSaleAmount;
        ...
        private void FillReturnByNumber()
        {
            #region Variables
            var Manager = new VoucherBLL();
            var SManager = new SalesHeadBLL();
            ...
            #region Filling Sales Header And Detail
            //List<VouchersEL> list = Manager.GetVouchersByTypeAndVoucherNumber(Operations.IdCompany, VoucherType, Convert.ToInt64(VEditBox.Text));
            List<VoucherDetailEL> ListSales = SManager.GetSalesReturnTransactionsByNumber(Validation.GetSafeLong(InvEditBox.Text), Operations.IdProject, Operations.BookNo, null);
            if (ListSales.Count > 0)
            {
                IdVoucher = ListSales[0].IdVoucher;
                InvEditBox.Text = Validation.GetSafeString(ListSales[0].VoucherNo);
        ...
        private void FillSales(List<VoucherDetailEL> List)
        {
            ...
            if (List != null && List.Count > 0)
            {
                ...
                for (int i = 0; i < List.Count; i++)
                {
                    ...
                    DgvStockSales.Rows[i].Cells[17].Value = CommonFunctions.RemoveTrailingZeros(List[i].Boxes);
                    DgvStockSales.Rows[i].Cells[18].Value = CommonFunctions.RemoveTrailingZeros(List[i].Pieces);
                    DgvStockSales.Rows[i].Cells[19].Value = List[i].UnitPrice;
                    DgvStockSales.Rows[i].Cells[20].Value = List[i].Amount;
                    //DgvSales.Rows[i].Cells[23].Value = List[i].Discount.ToString() + "%";
                    //DgvSales.Rows[i].Cells[24].Value = List[i].DiscountAmount;
                    //DgvSales.Rows[i].Cells[25].Value = List[i].BulkDiscountPercentage.ToString() + "%";
                    //DgvSales.Rows[i].Cells[26].Value = List[i].BulkDiscountFlatAmount;
                    //DgvSales.Rows[i].Cells[27].Value = List[i].BillAmountAfterDiscount;
                    DgvStockSales.Rows[i].Cells[21].Value = List[i].SaleTaxPercentage + "%";
                    DgvStockSales.Rows[i].Cells[22].Value = List[i].SaleTaxFlatAmount;
                    DgvStockSales.Rows[i].Cells[23].Value = List[i].OtherTaxPercentage + "%";
                    DgvStockSales.Rows[i].Cells[24].Value = List[i].OtherTaxFlatAmount;
