...
{
    public partial class frmPosPurchases : Form
    {
        #region Variables
        int EnterCounter = 0;
        ...
        public frmPosPurchases()
        {
            InitializeComponent();
        }
        private void frmPosPurchases_Load(object sender, EventArgs e)
        {
            LoadAllItems();           
                ...
                if (ValidateControls())
                {
                    if (ValidateBookPeriod())
                    {
                        ...
                        if (CustEditBox.Visible)
                        {
                            ...
                            {
                                oelSupplierTransaction.Description = "Purchases From : " + CustEditBox.Text;
                            }
                            if (CheckSoftwareTaxableNature())
                            {
                                oelSupplierTransaction.Credit = Validation.GetSafeDecimal(txtInvoiceTotalWithTax.Text);
                            }
                            else
                                oelSupplierTransaction.Credit = Validation.GetSafeDecimal(txtInvoiceTotalAfterDiscount.Text);
                        ...
                        if (!CustEditBox.Visible)
                        {
                            ...
                            oelCashTransaction.Description = "Cash Purchases From : ";

                            if (CheckSoftwareTaxableNature())
                            {
                                oelCashTransaction.Credit = Convert.ToDecimal(txtInvoiceTotalWithTax.Text);
                            }
                            else
                                oelCashTransaction.Credit = Convert.ToDecimal(txtInvoiceTotalAfterDiscount.Text);
                        ...
                        oelPurchaseTransaction.Debit = Validation.GetSafeDecimal(txtInvoiceTotalAfterDiscount.Text);
                        ...
                        if (CheckSoftwareTaxableNature())
                        {
                            ...
                            oelPurchaseTaxTransaction.Debit = Validation.GetSafeDecimal(txtTotalTaxAmount.Text);
                        ...
                        #endregion region
                        #region Saving Code
                        if (MessageBox.Show("Save Or Update Purchases Record?", "Saving Record", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
                        {
                            if (IdVoucher == null || IdVoucher == 0)
                            {
